...
def scrape_beneficiary(wd, beneficiary,contract_number):
    time.sleep(5)
    ...
    for b3_row in b_table:
        ...
        for b in b3:
            ...
            if len(bs)  == 8 and bs[1]=="Plan" and bs[3]=="Designation" and bs[-3]=="Percentage":
                ...
                result = [contract_number, None, b_item[-2], b_item[0], float(b_item[-1].strip('%')) / 100, None,
                          None, None, companies['EQ']]
                beneficiary.loc[len(beneficiary)] = result

            if len(bs) == 7 and bs[-2]=="Percentage":
                b_item = [bs[0], bs[4]]
                result = [contract_number, None, b_item[-1], b_item[0],None, None,None, None, companies['EQ']]
                beneficiary.loc[len(beneficiary)] = result

            if len(bs) == 7 and bs[-4]=="Designation":
                b_item = [bs[0], bs[-1]]
                result=[contract_number, None,None, b_item[0], float(b_item[-1].strip('%')) / 100, None,None, None, companies['EQ']]
                beneficiary.loc[len(beneficiary)] = result

            if len(bs) == 7 and bs[-5]=="Designation" and bs[-3]=="Percentage":
                b_item = [bs[0], bs[-4],bs[-2]]
                result = [contract_number, None, b_item[1], b_item[0], float(b_item[-1].strip('%')) / 100, None, None,
                          None, companies['EQ']]
                beneficiary.loc[len(beneficiary)] = result
            ...
            if len(bs) <= 3 or (len(bs)==4 and bs[1]=="Plan"):
                b_item = [bs[0]]
                result = [contract_number, None, None, b_item[0], None, None, None, None, companies['EQ']]
                beneficiary.loc[len(beneficiary)] = result
