...
def get_confs():
    ...
    """
    company, control_unit, maximum_iteration, thread_number, contract_file, head_mode, delete_flag = get_control(sys.argv)
...
def get_control(args):
    ...
    This method
    1. Company name
    2: mixed control modes for app, default mode is 1
    ...
    6. head_mode: Web-driver runs with window if True. Default: False
    7. delete_flag: delete the current data in SQL server if True. Default: True
    ...
    """
    # Due to the high usage of iA company, default is set to iA
    company = 'iA'
    control = 1
    max_iteration = 3
    thread_number = 1
    ...
    if len(args) > 1:
        if args[1] not in companies.values():
            print("Please Specify a valid Company name")
            exit()
        company = args[1]
    ...
    if len(args) > 4:
        thread_number = int(args[4])

    if len(args) > 5:
        file_name = args[5]

    if len(args) > 6:
        if args[6].lower() in ['h', 'head']:
            head_mode = True

    if len(args) > 7:
        if args[7].lower() in ['k', 'keep']:
            delete_flag = False

    return company, control, max_iteration, thread_number, file_name, head_mode, delete_flag
